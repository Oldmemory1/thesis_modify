...
\begin{table}[htbp]
	\centering
	\caption{Compressing and Packing Type}\label{tab:4.4}
	\begin{tabular*}{0.9\textwidth}{@{\extracolsep{\fill}}ccc}
		\toprule
		Disturbance Methods & Abbreviation & Effects \\
		\midrule
		upx\_pack & UP & using upx to compress ELF files \\
		upx\_unpack & UUP & using upx -d to decompress ELF files that have been compressed\\
		\bottomrule
	\end{tabular*}
\end{table}
...
\begin{table}[htbp]
	\centering
	\caption{Section Header Table Disturbance Type}\label{tab:4.5}
	\begin{tabular*}{0.9\textwidth}{@{\extracolsep{\fill}}ccc}
		\toprule
		Disturbance Methods & Abbreviation & Effects \\
		\midrule
		Remove Debug & RD & removing debugging information in binary files (filled with zeros) \\
		Break Checksum & BC & removing the checksum value in the optional header (filled with zeros) \\
		\bottomrule
	\end{tabular*}
\end{table}
...
\begin{algorithm}[htbp]
	\caption{ELF dynamically inserting algorithm using executable section to fill space}
    \KwIn{Target ELF Filepath \texttt{input\_elf},Payload Binary Code Path \texttt{code\_bin}, Output ELF Filepath \texttt{output\_elf}}
	\KwOut{An infected ELF file, inserted payload code}
...
\begin{algorithm}[htbp]
	\caption{ELF Adversarial Sample Generation Algorithm Based on RL}
	\KwIn{Origin ELF Malware Sample $S$,Maximum Iteration Times $I$,Pre-trained Models $M$(PPO+LSTM),Disturbance Action Table \texttt{Action\_table},Detection Result Record Table \texttt{Re}}
	\KwOut{Sample After Processing $S$}
	Init agent and environment env\;
	...
	\ForEach{$s \in S$}{
		...
		\For{$j \leftarrow 1$ \KwTo $I$}{
			...
			calculate evasion score:\texttt{Evasion\_Score} $\leftarrow$ score\_function($s$)\;
			
			calculate behavioral confusion degree:\texttt{Behavior\_Confusion\_Degree} $\leftarrow$ calculate\_confusion\_degree($s$)\;
			
			calculate disturbance cost:\texttt{Perturbation\_Cost} $\leftarrow$ calculate\_perturbation\_cost($s$, \texttt{Action\_table}[act])\;
